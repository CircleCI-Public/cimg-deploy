...
More specifically, the tag `2022.04.1` is used meaning this is the 1st April 2022 snapshot.
...
jobs:
  build:
    docker:
      - image: cimg/deploy:2022.04.1-node
    steps:
      - checkout
      - run: node --version
```
...
For example the Node.js variant is available, it can be used like this: `cimg/deploy:2022.04.1-node`.
...
```bash
./shared/gen-dockerfiles.sh 2022.04.1
```
...
```bash
cd 2022.04
docker build -t test/deploy:2022.04.1 .
docker run -it test/deploy:2022.04.1 bash
```
